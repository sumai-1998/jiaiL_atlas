...
def downsample_with_open3d(points, rgbs, target_count):
    """
    Downsample using Open3D Voxel Grid.
    Iteratively increases voxel size until count < target_count.
    """
    # 1. Convert Torch (GPU/CPU) -> Numpy (CPU) -> Open3D
    ...
    # 2. Iterative Voxel Downsampling
    # Heuristic: Estimate initial voxel size based on bounding box
    bbox = pcd.get_axis_aligned_bounding_box()
    extent = bbox.get_extent()
    # Start with a small size (e.g., 1/100th of the smallest dimension)
    voxel_size = min(extent) / 100.0 
    ...
    for _ in range(max_retries):
        if len(current_pcd.points) <= target_count:
            break
            
        # Apply downsampling
        current_pcd = pcd.voxel_down_sample(voxel_size=voxel_size)
        
        # If still too many points, increase voxel size aggressively
        if len(current_pcd.points) > target_count:
            # Increase voxel size by 20% each time
            voxel_size *= 1.1 
    
    # 3. Convert Open3D -> Numpy -> Torch (GPU)
    new_points = torch.from_numpy(np.asarray(current_pcd.points)).float().to(points.device)
    new_rgbs = torch.from_numpy(np.asarray(current_pcd.colors)).float().to(rgbs.device)
    
    return new_points, new_rgbs
