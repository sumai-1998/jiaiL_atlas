...
#
# --------------------------------------------------------
# modified from src.ttt3r.dust3r
...
class Attention(nn.Module):
    ...
    def forward(self, x, xpos, return_attn=False):
        ...
        if return_attn:
            ...
            x = (attn @ v).transpose(1, 2).reshape(B, N, C) # [B, N, C] [1, 768, 768]
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, qpos, kpos, return_attn=False):
        B, Nq, C = query.shape
        Nk = key.shape[1]
        Nv = value.shape[1]

        q = (
            self.projq(query) # [B, Nq, C] [1, 768, 768]
            .reshape(B, Nq, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        ) # [B, num_heads, Nq, C//num_heads] [1, 16, 768, 48]
        k = (
            self.projk(key) # [B, Nk, C] [1, 1 + 576, 768]
            .reshape(B, Nk, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        ) # [B, num_heads, Nk, C//num_heads] [1, 16, 1 + 576, 48]
        v = (
            self.projv(value) # [B, Nv, C] [1, 1 + 576, 768]
            .reshape(B, Nv, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        ) # [B, num_heads, Nv, C//num_heads] [1, 16, 1 + 576, 48]
        ...
        if self.rope is not None:
            if qpos is not None:
                ...
                with torch.autocast(device_type="cuda", enabled=False):
                    q = self.rope(q, qpos)
                q = q.to(q_type) # [B, num_heads, Nq, C//num_heads] [1, 16, 768, 48]

            if kpos is not None:
                k = k.float()
                with torch.autocast(device_type="cuda", enabled=False):
                    k = self.rope(k, kpos)
                k = k.to(k_type) # [B, num_heads, Nk, C//num_heads] [1, 16, 1 + 576, 48]
