...
from src.ttt3r.dust3r.heads.postprocess import (
    postprocess,
    postprocess_desc,
    postprocess_rgb,
...
from src.ttt3r.dust3r.utils.camera import pose_encoding_to_camera, PoseDecoder
from src.ttt3r.dust3r.blocks import ConditionModulationBlock
...
class DPTPts3dPose(nn.Module):
    ...
    def forward(self, x, img_info, **kwargs):
        if self.has_pose:
            pose_token = x[-1][:, 0].clone() # [1, 768]
            token = x[-1][:, 1:] # [1, 576, 768]
            with torch.amp.autocast(enabled=False, device_type="cuda"):
                pose = self.pose_head(pose_token) # [1, 7]

            token_cross = token.clone()
            for blk in self.final_transform:
                token_cross = blk(token_cross, pose_token, kwargs.get("pos")) # [1, 576, 768]
            x = x[:-1] + [token]
