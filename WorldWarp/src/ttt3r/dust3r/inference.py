import tqdm
import torch
from src.ttt3r.dust3r.utils.device import to_cpu, collate_with_cat
from src.ttt3r.dust3r.utils.misc import invalid_to_nans
from src.ttt3r.dust3r.utils.geometry import depthmap_to_pts3d, geotrf
...
def loss_of_one_batch(
    ...
    with torch.amp.autocast(enabled=not inference, device_type="cuda"):
        if inference:
...
@torch.no_grad()
def inference(groups, model, device, verbose=False):
    ignore_keys = set(
...
@torch.no_grad()
def inference_recurrent(groups, model, device, verbose=False):
    ignore_keys = set(
        ["depthmap", "dataset", "label", "instance", "idx", "true_shape", "rng"]
    )
    for view in groups:
...
@torch.no_grad()
def inference_recurrent_lighter(groups, model, device, verbose=False):
    if verbose:
        print(f">> Inference with model on {len(groups)} image/raymaps")
