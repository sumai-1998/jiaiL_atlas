...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, scale=1.0):
    ...
    def inpaint_setup(self, training_args, mask3d):
        ...
        def initialize_new_features(features, num_new_points, mask_xyz_values, distance_threshold=0.25, max_distance_threshold=1, k=5):
            ...
            if num_new_points == 0:
                for key in features:
                    new_features[key] = torch.empty(
                        (0, *features[key].shape[1:]), device=features[key].device)
                return new_features
            ...
            for key, feature in features.items():
                ...
                if feature_np.ndim == 2:
                    ...
                    raise ValueError(
                        f"Unsupported feature dimension: {feature_np.ndim}")
                new_points_np = np.mean(neighbor_points, axis=1)

                # Convert back to tensor
                new_features[key] = torch.tensor(
                    new_points_np, device=feature.device, dtype=feature.dtype)
        ...
        mask3d = ~mask3d.squeeze()  # True -> False
        mask_xyz_values = self._xyz[~mask3d]  # False -> True
        ...
        with torch.no_grad():
            new_xyz, new_features_dc, new_scaling, new_features_rest, new_opacity, new_rotation = initialize_new_features(
                sub_features, num_new_points, mask_xyz_values)
        ...
        self._xyz = nn.Parameter(torch.cat(
            [set_requires_grad(xyz_sub, False), set_requires_grad(new_xyz, True)]))
        self._features_dc = nn.Parameter(torch.cat([set_requires_grad(
            features_dc_sub, False), set_requires_grad(new_features_dc, True)]))
        self._features_rest = nn.Parameter(torch.cat([set_requires_grad(
            features_rest_sub, False), set_requires_grad(new_features_rest, True)]))
        self._opacity = nn.Parameter(torch.cat(
            [set_requires_grad(opacity_sub, False), set_requires_grad(new_opacity, True)]))
        self._scaling = nn.Parameter(torch.cat(
            [set_requires_grad(scaling_sub, False), set_requires_grad(new_scaling, True)]))
        self._rotation = nn.Parameter(torch.cat([set_requires_grad(
            rotation_sub, False), set_requires_grad(new_rotation, True)]))
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                group["params"][0] = nn.Parameter(torch.cat(
                    (group["params"][0].cuda(), extension_tensor), dim=0).requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, limit_num=-1):
        ...
        if limit_num > 0:
            ...
            sorted_pts_mask = torch.zeros(padded_grad.size(
                0), dtype=torch.bool, device=grads.device)
            inc_num = limit_num - self.get_num_pts
            if inc_num <= 0:
                return
            inc_num = min(inc_num, selected_pts_mask.sum().item())
            # inc_num = int(inc_num * random.random())
            sorted_pts_mask[idx[:inc_num]] = 1
            selected_pts_mask = torch.logical_and(
                selected_pts_mask, sorted_pts_mask)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, limit_num):
        ...
        if limit_num > 0:
            ...
            sorted_pts_mask = torch.zeros(grads.size(
                0), dtype=torch.bool, device=grads.device)
            inc_num = limit_num - self.get_num_pts
            if inc_num <= 0:
                return
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, limit_num=-1):
        ...
        if ((limit_num > 0) and (self.get_num_pts < limit_num)) or (limit_num < 0):
            self.densify_and_clone(
                grads, max_grad, extent, limit_num=limit_num)
