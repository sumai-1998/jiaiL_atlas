...
工作目录：`/data4/sumai/GIL_ATLAS`。创建：**2026-09-16**；最后更新：**2026-09-19**，时区 Asia/Shanghai。
...
按用户要求新增 [WorldWarp 原版与组合管线评测总结](docs/WORLDWARP_PIPELINE_EVALUATION_SUMMARY.md)，汇总同五场景上的原版、三项目与 F/G/H，共五种方法、25 个视频的已保存评测结果。包括参数与方法差异、六项指标定义、第 50 / 200 帧均值、逐场景改善数、结果解读、耗时、分段高斯资产和精简存储范围，并在仓库 README 添加入口。
...
按用户要求，将对话中展开的完整流程保存为 [docs/WORLDWARP_DETAILED_WORKFLOW.md](docs/WORLDWARP_DETAILED_WORKFLOW.md)。以已有 225 帧原版评测配置为例，记录首图与目标相机输入、TTT3R 深度、反投影与高斯初始化、500 次拟合、目标视角 RGB/有效掩码、VAE 与分区域噪声、视频联合去噪、历史帧重新建模和五段拼接；明确每段新建 GS、没有全局高斯持续更新，以及执行完成不代表质量复现成功。
...
更新 [WorldWarp 类 Atlas 小研究课题与模块替代路线](WorldWarp_类Atlas流程_小研究课题与模块替代路线_2026-09-14.md)，在原 12 项候选库前新增当前复审，保留历史建议并明确其优先级已被替代。依据原版每段新建 GS、已有几何有效性掩码和空间噪声调度、五场景组合结果及单场景重验失败，收敛为相机控制/重建一致性、可靠性驱动的条件保持、可撤销且带来源的持久记忆三个主方向；前馈 GS 和事件更新改为需先 profile 的效率方向。
