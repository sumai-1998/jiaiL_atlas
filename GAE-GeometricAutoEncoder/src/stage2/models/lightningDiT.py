...
class LightningDiTBlock(nn.Module):
    ...
    def forward(self, x, c, feat_rope=None, **block_kwargs):
        if self.wo_shift:
        ...
        # NormAttention requires total_view. Try to get it from kwargs.
        # If not present, default to 1 or infer from shape? 
        # NormAttention expects BV, N, C. total_view V.
        # If total_view is missing, we might assume single view (V=1)?
        # But for correctness, it should be passed.
        # In LightningDiT below, we will pass **kwargs which should contain total_view if passed from training.
        
        total_view = block_kwargs.get('total_view', 1) 
        # If total_view is not passed, NormAttention might fail or misbehave if x is (B, N, C). 
        # (B, N, C) with V=1 -> B=B//1. Correct.
        
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), 
                                                rope=feat_rope, 
                                                total_view=total_view,
                                                use_prope=self.use_prope, 
                                                viewmats=block_kwargs.get('viewmats'), 
                                                Ks=block_kwargs.get('Ks'),
                                                prope_image_size=block_kwargs.get('prope_image_size'))
...
class LightningDiT(nn.Module):
    ...
    def forward(self, x, t=None, y=None, **kwargs):
        ...
        for block in self.blocks:
            x = block(x, c, feat_rope=self.feat_rope, **kwargs)
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, cfg_interval=(-1e4, -1e4), interval_cfg: float = 0.0, **kwargs):
        """
    ...
    def forward_with_autoguidance(self, x, t, y, cfg_scale, additional_model_forward, cfg_interval=(-1e4, -1e4), interval_cfg: float = 0.0, **kwargs):
        """
        ...
        t = t[: len(t) // 2]
        y = y[: len(y) // 2]
        model_out = self.forward(half, t, y, **kwargs)
