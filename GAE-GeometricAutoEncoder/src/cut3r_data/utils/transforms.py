...
# Modified for GLD project - image transforms

import torch
import torchvision.transforms as tvf
import torchvision.transforms.functional as F
from cut3r_data.utils.image import ImgNorm
...
def _check_input(value, center=1, bound=(0, float("inf")), clip_first_on_zero=True):
    if isinstance(value, (int, float)):
        if value < 0:
            raise ValueError(f"If value is a single number, it must be non negative.")
