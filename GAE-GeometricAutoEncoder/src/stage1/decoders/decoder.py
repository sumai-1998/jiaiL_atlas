...
class GeneralDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        #     self.decoder_pos_embed = nn.ModuleList[]
        # else:
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int = None, width: int = None) -> torch.Tensor:
        """
        Interpolate position encodings for higher/variable resolutions.
        Should use 2D interpolation.
        """
        # -1 removes the class dimension
        num_positions = self.decoder_pos_embed.shape[1] - 1
        dim = self.decoder_pos_embed.shape[-1]
        
        # Source grid size (assumed square)
        h0 = w0 = int(num_positions**0.5)
        ...
        if height is None or width is None:
            # Fallback: Infer from embeddings length (square assumption fallback)
            # This is flawed for non-square, but maintains legacy behavior if args not provided
            target_seq_len = embeddings.shape[1] - 1
            h_t = w_t = int(target_seq_len**0.5)
        else:
            h_t, w_t = height, width

        # Reshape to 2D grid: (B, C, H, W) -> (1, dim, h0, w0)
        # patch_pos_embed is (1, num_pos, dim)
        ...
        # Flatten back: (1, dim, h_t, w_t) -> (1, h_t*w_t, dim)
    ...
    def forward(
        ...
        if interpolate_pos_encoding:
             # Skip latent interpolation, trust input shape
             if drop_cls_token:
                 x_ = x[:, 1:, :]
             else:
                 x_ = x
        ...
        # add pos embed
        if interpolate_pos_encoding:
            decoder_pos_embed = self.interpolate_pos_encoding(x, height=patches_h, width=patches_w)
        else:
...
class GeneralDecoder_Variable(nn.Module):
    def __init__(self, config, base_image_size: Tuple[int, int]):
        super().__init__()
        self.config = config

        self.decoder_embed = nn.Linear(config.hidden_size, config.decoder_hidden_size, bias=True)

        # Base grid is determined by init-time base_image_size
        p = config.patch_size
        h0, w0 = base_image_size
        if (h0 % p) != 0 or (w0 % p) != 0:
            raise ValueError(f"base_image_size {base_image_size} must be divisible by patch_size={p}")
        self.base_grid = (h0 // p, w0 // p)  # (H0p, W0p)
        self.base_num_patches = self.base_grid[0] * self.base_grid[1]

        # Frozen base pos-embed
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, self.base_num_patches + 1, config.decoder_hidden_size),
            requires_grad=False,
        )
    ...
    def initialize_weights(self):
        # If your get_2d_sincos_pos_embed only supports square grids, keep base_grid square.
        h0p, w0p = self.base_grid
        if h0p != w0p:
            raise ValueError(
                "Your get_2d_sincos_pos_embed likely assumes square grid. "
                "Either keep base_image_size square or implement rectangular sin-cos init."
            )
        pe = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], h0p, add_cls_token=True)
    ...
    def interpolate_pos_encoding(self, target_grid: Tuple[int, int]) -> torch.Tensor:
        """
        Interpolate base pos-embed (H0p,W0p) -> target (Hp,Wp).
        Returns: (1, 1 + Hp*Wp, D)
        """
        hp, wp = target_grid
        h0p, w0p = self.base_grid
        ...
        dim = self.decoder_pos_embed.shape[-1]
        cls_pos = self.decoder_pos_embed[:, :1, :]     # (1,1,D)
        patch_pos = self.decoder_pos_embed[:, 1:, :]   # (1,H0p*W0p,D)

        # sanity check
        if patch_pos.shape[1] != h0p * w0p:
            raise ValueError(
                f"Base pos-embed patches {patch_pos.shape[1]} != base_grid product {h0p*w0p}."
            )

        patch_pos = patch_pos.reshape(1, h0p, w0p, dim).permute(0, 3, 1, 2)  # (1,D,H0p,W0p)
        patch_pos = nn.functional.interpolate(
            patch_pos, size=(hp, wp), mode="bicubic", align_corners=False
        )
        patch_pos = patch_pos.permute(0, 2, 3, 1).reshape(1, hp * wp, dim)   # (1,Hp*Wp,D)
    ...
    def forward(
        ...
        target_grid = (h // p, w // p)                 # (Hp, Wp)
        target_num_patches = target_grid[0] * target_grid[1]

        # If input has CLS, drop it; otherwise keep all tokens as patches
        x_tokens = x[:, 1:, :] if drop_cls_token else x  # (B, N, Ddec)

        # Hard check: patch token count must match target grid
        if x_tokens.shape[1] != target_num_patches:
            raise ValueError(
                ...
                "Either pass the correct input_size, or add token interpolation, or fix the upstream tokenization."
            )

        # Prepend our trainable CLS
        cls_token = self.trainable_cls_token.expand(x_tokens.shape[0], -1, -1)
        x = torch.cat([cls_token, x_tokens], dim=1)  # (B, 1+N, Ddec)

        # Positional embedding (base -> target)
        if interpolate_pos_encoding:
            decoder_pos_embed = self.interpolate_pos_encoding(target_grid)  # must return (1, 1+N, D)
        else:
            decoder_pos_embed = self.decoder_pos_embed
            if decoder_pos_embed.shape[1] != x.shape[1]:
                raise ValueError("interpolate_pos_encoding=False requires base grid == target grid.")

        decoder_pos_embed = decoder_pos_embed.to(device=x.device, dtype=x.dtype)
        hidden_states = x + decoder_pos_embed
